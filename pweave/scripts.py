...
def weave():
    ...
    parser = OptionParser(
        usage="pweave [options] sourcefile", version="Pweave " + pweave.__version__
    )
    parser.add_option(
        "-f",
        "--format",
    ...
    parser.add_option(
        "-o", "--output", dest="output", default=None, help="Name of the output file"
    )
    parser.add_option(
        "-l",
        "--list-formats",
...
def publish():
    ...
    parser = OptionParser(
        usage="pypublish [options] sourcefile",
        version="Part of Pweave " + pweave.__version__,
    )
    parser.add_option(
        "-f",
        "--format",
    ...
    parser.add_option(
        "-t", "--theme", dest="theme", default="skeleton", help="Theme for HTML output"
    )
    parser.add_option(
        "-o", "--output", dest="output", default=None, help="Name of the output file"
    )
    ...
    pweave.publish(
        infile, options.format, options.theme, options.latex_engine, options.output
    )
...
def tangle():
    ...
    parser = OptionParser(
        usage="ptangle sourcefile", version="Pweave " + pweave.__version__
    )
    parser.add_option(
        "-i",
        "--input-format",
...
def convert():
    ...
    parser = OptionParser(
        usage="pweave-convert [options] sourcefile",
        version="Part of Pweave " + pweave.__version__,
    )
    parser.add_option(
        "-i",
