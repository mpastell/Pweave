...
class PwebPandocFormatter(PwebFormatter):
    ...
    def formatfigure(self, chunk):
        ...
        if chunk["caption"] and len(fignames) > 0:
            if len(fignames) > 1:
                print(
                    "INFO: Only including the first plot in a chunk when the caption is set"
                )

            figstring = self.make_figure_string(
                fignames[0], chunk["width"], chunk["name"], caption
            )
            return figstring
