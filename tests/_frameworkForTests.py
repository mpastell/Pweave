...
class RegressionTest(ParametricTests):
    ...
    def assertSameAsReference(self, reference=None):
        try:
            self.assertEqual(
                self.contentOf(reference if reference is not None else self.REFERENCE),
                self.contentOf(self.OUTFILE),
            )
        except AssertionError:
            raise AssertionError(
                "{ref} and {out} differs\ntry:\n$ vimdiff {ref} {out}".format(
                    ref=self.absPathTo(self.REFERENCE), out=self.absPathTo(self.OUTFILE)
                )
            )
    ...
    def assertSameAsPattern(self, __pattern=None, **kwargs):
        try:
            self.assertEqual(
                self.contentOf(__pattern).format(**kwargs), self.contentOf(self.OUTFILE)
            )
        except AssertionError:
            raise AssertionError(
                "{ref} and {out} differs\ntry:\n$ vimdiff {ref} {out}".format(
    ...
    def absPathTo(self, filename):
        return os.path.abspath(
            os.path.join(os.path.dirname(__file__), self.TESTDIR, filename)
        )
